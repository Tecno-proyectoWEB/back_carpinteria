            ...
            <div class="max-w-5xl mx-auto">
                <div class="bg-white shadow-sm rounded-lg overflow-hidden">
                    ...
                    <div class="px-6 py-4 bg-gray-50 border-b border-gray-200 flex justify-between items-center">
                        <div>
                            <h2 class="text-2xl font-bold text-gray-900">Pedido #{{ pedido?.id || 'N/A' }}</h2>
                            <p v-if="pedido?.fecha" class="text-sm text-gray-500 mt-1">
                        ...
                        <div class="mb-6">
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                ...
                                <div>
                                    ...
                                    <p class="text-gray-900">{{ pedido?.metodoPago?.nombre || 'N/A' }}</p>
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="border rounded-lg overflow-hidden">
                                <table class="min-w-full divide-y divide-gray-200">
                                    ...
                                    <tbody class="bg-white divide-y divide-gray-200">
                                        <tr v-for="detalle in (pedido?.detalles || [])" :key="detalle?.id || Math.random()">
                                            ...
                                            <td class="px-4 py-3 text-sm">{{ detalle?.cantidad || 0 }}</td>
                                            <td class="px-4 py-3 text-sm">${{ parseFloat(detalle?.precio_unitario || 0).toFixed(2) }}</td>
                                            <td class="px-4 py-3 text-sm font-semibold">
                                                ${{ parseFloat(detalle?.importe_total_desc || 0).toFixed(2) }}
                                            </td>
                                        </tr>
                                    </tbody>
                                    <tfoot class="bg-gray-50">
                                        <tr>
                                            ...
                                            <td class="px-4 py-3 text-xl font-bold text-green-600">
                                                ${{ parseFloat(pedido?.importe_total_desc || 0).toFixed(2) }}
                                            </td>
                                        </tr>
                                    </tfoot>
                                </table>
                        ...
                        <div v-if="pedido?.pagos && pedido.pagos.length > 0">
                            ...
                            <div class="border rounded-lg overflow-hidden">
                                <table class="min-w-full divide-y divide-gray-200">
                                    ...
                                    <tbody class="bg-white divide-y divide-gray-200">
                                        <tr v-for="pago in pedido.pagos" :key="pago?.id || Math.random()">
                                            <td class="px-4 py-3 text-sm font-semibold">
                                                ${{ parseFloat(pago?.monto || 0).toFixed(2) }}
                                            </td>
                                            <td class="px-4 py-3 text-sm">{{ pago?.tipo || 'N/A' }}</td>
                                            <td class="px-4 py-3 text-sm">
                                                <Badge
