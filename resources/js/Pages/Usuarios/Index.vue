...
// Función route segura usando window.route (Ziggy global)
const getRoute = (name, params = null) => {
    try {
        ...
        // Intentar acceder directamente (si está en el scope global)
...
const viewUsuario = (usuario) => {
    if (!usuario || !usuario.id) {
        console.error('Usuario inválido para ver:', usuario);
        return;
    }
    try {
...
const submitCreate = () => {
    ...
    // Convertir rol_id a número si es string
...
const deleteUsuario = (usuario) => {
    if (!usuario || !usuario.id) {
        console.error('Usuario inválido para eliminar:', usuario);
        return;
    }
...
const editUsuario = (usuario) => {
    if (!usuario || !usuario.id) {
        console.error('Usuario inválido para editar:', usuario);
        return;
    }
    try {
