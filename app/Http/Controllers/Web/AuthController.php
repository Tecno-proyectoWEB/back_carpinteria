...
class AuthController extends Controller
{
    ...
    public function login(Request $request)
    {
        ...
        if (!$usuario) {
            Log::warning('Intento de login fallido: usuario no encontrado', ['email' => $request->email]);
        ...
        $passwordCheck = Hash::check($request->password, $passwordHash);

        if (!$passwordCheck) {
            Log::warning('Intento de login fallido: contraseña incorrecta', [
                'email' => $request->email,
                'password_hash_preview' => substr($passwordHash, 0, 20) . '...'
            ]);
            return back()->withErrors([
                'message' => 'Credenciales inválidas',
            ])->withInput($request->only('email'));
        ...
        if (!Auth::check()) {
            Log::error('Error al autenticar usuario después de login', [
                'user_id' => $usuario->id,
                'email' => $usuario->email
            ]);
            return back()->withErrors([
        ...
        // Para Inertia, usar redirect normal en lugar de intended
        // Si hay una URL previa, redirigir allí, sino al dashboard
        $intendedUrl = $request->session()->pull('url.intended', route('dashboard'));

        // Si es una petición de Inertia, usar Inertia::location para forzar redirección HTTP
